...
■ Today（📍）
  ...
  },
  {
    title: 'スケジュールライン',
    content: `■ スケジュールラインとは
  マイルストーンから垂直線を引いて、期限やイベントを
  全レーンに横断表示する機能です。

■ 追加方法
  マイルストーンを右クリック →「スケジュールラインを追加」

■ カスタマイズ
  ・色: 任意のカラーを指定可能
  ・太さ: strokeWidth で線の太さを変更
  ・線種: 実線 / 破線 / 点線から選択
  ・ラベル: 線に説明テキストを付与`,
  },
  {
    title: 'テキストボックス',
    content: `■ テキストボックスとは
  チャート上に注記やコメントを自由に配置する機能です。

■ 追加方法
  ツールバーの「+ ▼」→「TextBox」で追加します。

■ カスタマイズ
  ・テキスト色、背景色、枠線色、枠線太さ
  ・フォントサイズ
  ・矢印（arrow）: 特定のバーやマイルストーンを
    指し示す矢印を設定可能`,
  },
  {
    title: 'エクスポート・インポート',
  ...
  ・部分エクスポート: 特定ページだけの JSON 出力も可能

■ インポート
  「⋯ ▼」→「Import」で JSON ファイルを読み込みます。
  ページ名やレーンが既存データと衝突する場合は、
  衝突解決UI で「追加 / 上書き / スキップ」を選択できます。`,
  },
  {
    title: 'その他の機能',
    content: `■ Undo / Redo
  ↩ / ↪ ボタンで操作の取消・やり直しが可能です（50ステップ）。
...
■ 複数ページ
  ...
  ページごとにタイムライン範囲・月幅を個別設定できます。

■ レーンテンプレート
  「+ ▼」→「レーン管理」でテンプレートの管理が可能です。
  テンプレートからレーンを追加すると、タグ・高さの設定が引き継がれ、
  複数ページ間でレーン構成を統一できます。

■ マイルストーンのカスタマイズ
  ・色やサイズを個別に変更可能
  ・★アイコンとテキストを独立してドラッグ移動
  ・Set ボタンで位置を日付にスナップ
